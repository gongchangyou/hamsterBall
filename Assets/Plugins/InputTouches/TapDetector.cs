...
public class TapDetector : MonoBehaviour {
	...
	void Start () {
		...
		if(enableMultiTapFilter){
			Gesture.SetMultiTapFilter(enableMultiTapFilter);
			Gesture.SetMaxMultiTapCount(maxMultiTapCount);
			Gesture.SetMaxMultiTapInterval(multiTapInterval);
		}
	...
	void CheckMultiTapMouse(int index, Vector2 startPos, Vector2 lastPos){
		if(multiTapMouse[index].lastTapTime>Time.realtimeSinceStartup-multiTapInterval){
			if(Vector2.Distance(startPos, multiTapMouse[index].lastPos)<multiTapPosSpacing){
				...
				Gesture.MultiTap(new Tap(startPos, lastPos, multiTapMouse[index].count, index, true));
				...
				Gesture.MultiTap(new Tap(startPos, lastPos, 1, index, true));
			}
		}
		else{
			multiTapMouse[index].count=1;
	...
	void CheckMultiTapTouch(int index, Vector2 startPos, Vector2 lastPos){
		...
		if(multiTapTouch[index].lastTapTime>Time.realtimeSinceStartup-multiTapInterval){
			if(Vector2.Distance(startPos, multiTapTouch[index].lastPos)<multiTapPosSpacing){
				multiTapTouch[index].count+=1;
				multiTapTouch[index].lastPos=startPos;
				multiTapTouch[index].lastTapTime=Time.realtimeSinceStartup;
				
				Gesture.MultiTap(new Tap(startPos, lastPos, multiTapTouch[index].count, index, false));
				...
				Gesture.MultiTap(new Tap(startPos, lastPos, 1, index, false));
			}
		}
		else{
			multiTapTouch[index].count=1;
